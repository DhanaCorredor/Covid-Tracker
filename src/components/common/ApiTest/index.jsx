...
function Row({ label, state }) {
  if (state.loading) return <tr><td>{label}</td><td>⏳ cargando...</td></tr>
  if (state.error)   return <tr><td>{label}</td><td style={{ color: 'red' }}>❌ {state.error}</td></tr>
  return               <tr><td>{label}</td><td style={{ color: 'green' }}>✅ OK</td></tr>
}
...
export function ApiTest() {
  ...
  return (
    <div style={{ padding: '1.5rem', fontFamily: 'monospace', fontSize: '0.85rem' }}>
      ...
      <table border="1" cellPadding="8" style={{ borderCollapse: 'collapse', marginBottom: '2rem' }}>
        <thead>
      ...
      {paises.data.length > 0 && (
        <section>
          ...
          {paises.data.slice(0, 3).map((c) => (
            <div key={c.country} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.25rem' }}>
              <img src={getFlagUrl(c.countryInfo?.iso2)} alt={c.country} width={24} />
              <span>{c.country}: {c.cases?.toLocaleString()} casos</span>
            </div>
          ))}
        </section>
      )}
      ...
      {historico.data && (
        <section>
          ...
          <ul>
            {Object.entries(historico.data.timeline?.cases ?? {}).slice(0, 3).map(([fecha, valor]) => (
              <li key={fecha}>{fecha}: {valor?.toLocaleString()} casos</li>
            ))}
          </ul>
        </section>
      )}
